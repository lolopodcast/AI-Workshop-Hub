...
與各份教材 SPA 同一套規格，讓整個教材群的行為一致：

- **右鍵**：跳出深綠色藥丸提示 `Intellectual Property of Prof. Shihmin Lo`，2 秒自動消失。
- **快捷鍵**：`F12`、`Ctrl+U`、`Ctrl+Shift+I/J/C` 一律改為顯示版權提示。
- **圖片與 SVG**：禁止拖曳另存。
- **選取**：頁首、分類列、頁尾、按鈕等介面裝飾不可反白；**教材標題與說明文字保持可選取、可複製**。

刻意保留不擋的情況（擋了只會惹惱使用者，並不會多保護什麼）：

- **輸入框內的右鍵**：登入密碼欄、搜尋框照常可貼上。
- **已反白選取的文字**：使用者刻意選取後右鍵，照常出現原生選單。
- **`Ctrl+P` 與 `Ctrl+S`**：維持可列印、可存檔。
- **`admin.html` 不套用**：那是您自己的編輯工具，需要右鍵複製貼上 JSON。

> ⚠️ 這是**版權宣示的禮貌提示，不是防拷貝**。看得懂原始碼的人一定繞得過去。
> 真正不想外流的內容請用 `published: false` 下架，不要指望擋右鍵。
